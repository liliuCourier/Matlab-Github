...
% 计算压差
L_total = L + 30*D;   % 简单考虑弯道带来的局部阻力损失
dp_f_CV =   (f_CV*L_total/CV_num).*(mdot_CV.^2)./(2*Dtube_CV*D*S^2);    % Pa  摩擦压损
dp_v_CV =   16*mdot_CV.^2/(pi^2*D^4).*(1./Dout_CV - 1./Din_CV);   % Pa  速度压损
...
%% 残差归一化预处理
% 根据进口管的两个饱和状态来预测物性
[D_inlet, Pr_inlet, Nu_inlet, T_inlet, x_inlet, k_inlet, isTP_inlet, vsatliq_inlet, vsatvap_inlet, Prsatliq_inlet, Prsatvap_inlet, Nusatliq_inlet, Nusatvap_inlet, ksatliq_inlet, ksatvap_inlet] = Prop(p_inlet, h_inlet);

% 计算饱和液相和饱和气相下的流速和雷诺数
veloctiy_liquid_ref = (mdot_inlet*vsatliq_inlet)/S;
veloctiy_vap_ref = (mdot_inlet*vsatvap_inlet)/S;

Re_liquid_ref = veloctiy_liquid_ref*D./(Nusatliq_inlet*1e-6);
Re_vap_ref = veloctiy_vap_ref*D./(Nusatvap_inlet*1e-6);
f_ref=      (-1.8*log10(6.9./min(Re_vap_ref,Re_liquid_ref)+(r/3.7)^1.11)).^(-2);

% 计算参考压降
dp_ref = (f_ref*L_total/CV_num/D)*((0.5*veloctiy_vap_ref+0.5*veloctiy_liquid_ref)^2/(vsatliq_inlet+vsatvap_inlet));


%% 残差输出
% 前面这两个质量流量守恒方程的归一化应该是没有问题的，只有在支路特别多的时候可能会出问题，
% 但是整体的量级能够保持在0.1~1
